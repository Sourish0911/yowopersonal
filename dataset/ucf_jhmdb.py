...
class UCF_JHMDB_Dataset(Dataset):
    def __init__(self,
        ...
        if self.is_train:
            ...
            self.split_list = 'trainlist_4.txt'
        else:
            # self.split_list = 'testlist.txt'
            self.split_list = 'testlist_4.txt'
    ...
    def pull_item(self, index):
        ...
        # image name
        # img_id = int(img_split[-1][:5]) # Commented by akshad 31/03
        img_id_tmp = img_split[-1].split(".")[0]
        img_id = int(img_id_tmp)

        # path to label
        #label_path = os.path.join(self.data_root, img_split[0], img_split[1], img_split[2], '{:05d}.txt'.format(img_id))
        label_path = os.path.join(self.data_root, img_split[0], img_split[1], img_split[2], '{}.txt'.format(img_id_tmp))
        ...
        for i in reversed(range(self.len_clip)):
            ...
            if self.dataset == 'ucf24':
                ...
                path_tmp = os.path.join(self.data_root, 'rgb-images', img_split[1], img_split[2] ,'{}.jpg'.format(img_id_tmp))
